...
export class RegisterFormComponent extends LitElement {
		...
		if (!this.emailAsUsername) {
			...
			if (this.username.length == 0) {
				...
				input.setCustomValidity(
					this.emailAsUsername
						? "Username must be a valid email address."
						: "Username can only contain alphanumerical characters.",
				);
				input.reportValidity();
				input.parentElement.classList.add("error");
				fail = true;
			...
			case 200:
				...
				setTimeout(() => {
					window.location.href = `/login${
						this.hasOnboarding == "true" ? "?onboard=true" : ""
					}`;
				}, 1000);
				break;
			case 400:
				...
				if (json["error"] !== undefined) {
					switch (json["error"]) {
						...
						default:
							alert(
								"Email does not match invitation email. Please reload and try again.",
							);
					}
				}
				break;
	...
	render() {
		return html`<div id="root">
			${this.showHIBPWarning
				? html`<section id="hibpWarning">
						<p id="warning">
							${this.bypassHIBPWarning
								? "Bypassing Security Warning"
								: `Account Security ${
										this.hibp === "loose" ? "Warning" : "Alert"
									}`}
						</p>
						${this.bypassHIBPWarning
							? html`
									<p>
										You are being registered with an insecure password. You will
										be redirected to the login page momentarily.
									</p>
								`
							: html`
									<p>
										The password you tried to use has been used before and is no
										longer safe because it was shared publicly online after a
										theft.
										<strong
											>Don't worry, this didn't happen on our website.</strong
										>
										To keep your account safe, we
										${this.hibp != "strict" ? "really " : ""}need you to pick a
										new password. Thank you for understanding and helping us
    ...
    ${
      ...
      <div class="input${this.registrationError == 2 ? " error" : ""}">
        ...
        <input id="email" type="email" placeholder="Email" autocorrect="off" autocapitalize="off" value="${
					this.email
				}" @input="${this.updateEmail}" ?disabled=${this.emailDisabled} />
      </div>
      <div class="input">
          <label for="password">Password</label>
          <input id="password" type="password" placeholder="Password" autocorrect="off" autocapitalize="off" value="${
						this.password
					}" @input="${this.updatePassword}" />
      </div>
      <div class="input${this.registrationError == 1 ? " error" : ""}">
          <label for="confPassword">Confirm Password</label>
          <input id="confPassword" type="password" placeholder="Confirm Password" autocorrect="off" autocapitalize="off" value="${
						this.confirmedPassword
					}" @input="${this.updateConfirmedPassword}" />
      </div>

      <button style="--color: ${this.backgroundColor};" @click="${
				this.register
			}">${this.registering ? "Registering..." : "Register"}</button>

      <p id="error">${this.getRegistrationErrorMessage()}</p>
      ${
				this.registrationSuccess
					? html`<p id="success">
							User registered, redirecting to login page..
						</p>`
					: ""
			}
      </div>
